...
class ventana_compresor(ctk.CTk):
    ...
    def comprimir(self, direccion):
        file_name = basename(direccion)
        image_cv2 = imread(direccion)
        altura, ancho, _ = image_cv2.shape

        nancho = int((self.estado_init_reduc/100) * ancho)
        nalto = int((self.estado_init_reduc/100) * altura)
        imagen_reduc = resize(image_cv2, (nancho, nalto))
        imagen_pill = Image.fromarray(cvtColor(imagen_reduc, COLOR_BGR2RGB))
    ...
    def ejecutar_compress(self):
        files = listdir(self.directorio_de_origen)
        jpg_files = [file for file in files if file.endswith('.jpg')]
        jpg_files =  jpg_files + [file for file in files if file.endswith('.JPG')]
        self.dataprint("Encontrados {} archivos".format(len(jpg_files)),1)
